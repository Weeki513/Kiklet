...
fn code_to_vk(code: &str) -> Option<u32> {
    match code {
        ...
        "KeyY" => Some(0x59), "KeyZ" => Some(0x5A), "Digit1" | "1" => Some(0x31),
        "Digit2" | "2" => Some(0x32), "Digit3" | "3" => Some(0x33), "Digit4" | "4" => Some(0x34),
        "Digit5" | "5" => Some(0x35), "Digit6" | "6" => Some(0x36), "Digit7" | "7" => Some(0x37),
        "Digit8" | "8" => Some(0x38), "Digit9" | "9" => Some(0x39), "Digit0" | "0" => Some(0x30),
        "Space" => Some(0x20), "Enter" => Some(0x0D), "Tab" => Some(0x09), "Escape" => Some(0x1B),
        "Backspace" => Some(0x08), "Delete" => Some(0x2E), "Insert" => Some(0x2D),
        "Home" => Some(0x24), "End" => Some(0x23), "PageUp" => Some(0x21), "PageDown" => Some(0x22),
        "ArrowUp" => Some(0x26), "ArrowDown" => Some(0x28), "ArrowLeft" => Some(0x25),
        "ArrowRight" => Some(0x27), "F1" => Some(0x70), "F2" => Some(0x71), "F3" => Some(0x72),
        "F4" => Some(0x73), "F5" => Some(0x74), "F6" => Some(0x75), "F7" => Some(0x76),
        "F8" => Some(0x77), "F9" => Some(0x78), "F10" => Some(0x79), "F11" => Some(0x7A),
        "F12" => Some(0x7B),
        _ => {
...
struct PttState {
    ...
    key_pressed: Arc<Mutex<bool>>,
}

static STATE: OnceLock<Arc<Mutex<Option<PttState>>>> = OnceLock::new();
static HOOK_USER_DATA: OnceLock<Arc<Mutex<Option<(PttConfig, AppHandle)>>>> = OnceLock::new();
                        ...
                        if let Some(code) = &config.code {
                            if let Some(target_vk) = code_to_vk(code) {
                                if vk_code == target_vk {
                                    ...
                                    let mods_match = if let Some(mods) = &config.mods {
                                        (!mods.ctrl || ctrl_down) &&
                                        (!mods.alt || alt_down) &&
                                        (!mods.shift || shift_down) &&
                                        (!mods.cmd || win_down)
                                    } else {
                                        !ctrl_down && !alt_down && !shift_down && !win_down
                                    };
                                        ...
                                        if let Some(state_arc) = STATE.get() {
                                            if let Ok(guard) = state_arc.lock() {
                                                if let Some(state) = guard.as_ref() {
                                                    if !is_up {
                                                        let was_pressed = if let Ok(mut pressed) = state.key_pressed.lock() {
                                                            let was = *pressed;
                                                            *pressed = true;
                                                            was
                                                        } else {
...
fn message_loop_thread(app: AppHandle, config: PttConfig, stop_flag: Arc<Mutex<bool>>) {
    unsafe {
        ...
        if hook == 0 {
            eprintln!("[kiklet][ptt][win] SetWindowsHookExW failed");
            return;
        }
...
pub fn start(app: &AppHandle, config: PttConfig) -> Result<(), String> {
    ...
    eprintln!("[kiklet][ptt][win] start accelerator={}", config.accelerator);
    ...
    eprintln!("[kiklet][ptt][win] start ok");
    Ok(())
}
    ...
    if let Some(state_arc) = STATE.get() {
        if let Ok(mut guard) = state_arc.lock() {
            if let Some(state) = guard.take() {
                ...
                eprintln!("[kiklet][ptt][win] stop ok");
            }
        }
    }
    Ok(())
